...
function trueModel(x) { return 1 + 0.5 * x + 0.2 * x ** 2 + JXG.Math.Statistics.randomNormal(0, 2) };

const y = x.map(trueModel);

function alphaVal() { return alpha.Value(); };
function beta1Val() { return beta1.Value(); };
function beta2Val() { return beta2.Value(); };
